...
- 工作区内的 Claude Code 项目级配置会影响它的行为。
...
- `设置`：修改助手名称和问候语、导入 Codex 桌宠、选择 Agent provider、选择工作区、调整 Claude Code 权限、开关剪贴板感知。
- `任务历史`：查看最近任务、状态和摘要。

设置保存后会立即刷新运行时配置：Agent Bridge 会按新 provider 和工作区重建，剪贴板监听会按新开关重启，桌宠和问候语会从新配置读取。

开发环境下，设置会写入项目内的 `config/` 与 `src/renderer/public/`；安装后的正式应用会写入 Electron `userData` 目录，避免安装目录不可写。

导入 Codex 桌宠时，选择包含 `pet.json` 和 `spritesheet.webp` 的文件夹即可。应用会复制该文件夹并更新 `manifest.json`，不需要手动切图。
